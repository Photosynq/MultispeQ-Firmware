...
SHUTDOWN = eeprom->SHUTDOWN_value;  //(3 * 60 * 60 * 1000); 
if (SHUTDOWN==0) {
  SHUTDOWN=(3 * 60 * 60 * 1000);  //if there is no value for SHUTDOWN then set to the default of 3 hours
  store(SHUTDOWN_value, SHUTDOWN);
}


WHEN_USB_ON_STAY_ON=eeprom->SHUTDOWN_value;
...
//  delay(100);                   // let battery voltage stabilize - if there are no other delays ahead of it then this can be deleted.
  ...
  serial_buffer=new char[serial_buffer_size + 1];     // large buffer for reading in a json protocol from serial port
 ...
 if (isnan(eeprom->open_thickness) || isnan(eeprom->closed_thickness)){  //if the falue of open_thickness is less than 100, it is not yet set correcetly
     ...
     store(closed_thickness, eeprom->thickness_min + 0.2*(eeprom->thickness_max-eeprom->thickness_min));  //10% from closed to fully open
...
pump_working=0; //start with the default that the pump is not connected.
